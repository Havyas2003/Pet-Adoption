...
<head>
  ...
  <title>Home Pets - Edit Status</title>
  ...
  <main id="main">
    ...
    <div class="container" data-aos="fade-up">
        ...
        <?php 
            ...
            $sql = ("SELECT * FROM adoptionform WHERE formID = $id");  
            $result = $conn->query($sql);
            if ($result->num_rows > 0) {
            // output data of each row
            while($row = $result->fetch_assoc()) {
                echo "<div  data-aos='fade-up' data-aos-delay='100'>";
                echo "<p>Name of User: " . $row["username"] . "</p>";
                echo "<p>Name of Animal: " . $row["animalName"] . "</p>";
                echo "<form action='editStatus.php' method='post'>";
                echo "<div class='form-group'>
                <select class='form-select' name='formID'>
                  <option selected>" . $row["formID"] . "</option>
                </select>
                <select class='form-select' name='formStatus'>
                  <option selected>" . $row["formStatus"] . "</option>
                  <option value='Accepted'>Accepted</option>
                  <option value='Rejected'>Rejected</option>
                </select>
                <button type='submit' name='editStatus'>Edit</button>
              </div></form>";
                echo "</div>";
            }
            } 
